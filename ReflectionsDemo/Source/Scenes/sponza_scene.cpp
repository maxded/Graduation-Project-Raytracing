...
void SponzaScene::Load(const std::string& filename)
{
	...
	// Create a root signature for the sponza scene.
	{
	...
	// Create pipeline state object with shader permutations for sponza scene.
	{
		...
		ThrowIfFailed(device->CreatePipelineState(&hdr_pipeline_state_stream_desc, IID_PPV_ARGS(&sponza_pipeline_state_map_[ShaderOptions::USE_AUTO_COLOR])));
