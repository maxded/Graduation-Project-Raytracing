...
void StaticMesh::Render(CommandList& commandList, uint32_t instanceCount, uint32_t firstInstance)
{
...
std::unique_ptr<StaticMesh> StaticMesh::CreateSphere(CommandList& commandList, float diameter, size_t tessellation, bool rhcoords)
{
	...
	for (size_t i = 0; i <= verticalSegments; i++)
	{
		...
		for (size_t j = 0; j <= horizontalSegments; j++)
		{
			...
			vertices.push_back(VertexData(normal * radius, normal, textureCoordinate));
		}
	}
...
std::unique_ptr<StaticMesh> StaticMesh::CreateCube(CommandList& commandList, float size, bool rhcoords)
{
...
static void CreateCylinderCap(VertexCollection& vertices, IndexCollection& indices, size_t tessellation, float height, float radius, bool isTop)
{
	...
	for (size_t i = 0; i < tessellation; i++)
	{
		...
		vertices.push_back(VertexData(position, normal, textureCoordinate));
	}
}

std::unique_ptr<StaticMesh> StaticMesh::CreateCone(CommandList& commandList, float diameter, float height, size_t tessellation, bool rhcoords)
{
	VertexCollection vertices;
	IndexCollection indices;
	...
	for (size_t i = 0; i <= tessellation; i++)
	{
		...
		vertices.push_back(VertexData(topOffset, normal, g_XMZero));
		vertices.push_back(VertexData(pt, normal, textureCoordinate + g_XMIdentityR1));
...
std::unique_ptr<StaticMesh> StaticMesh::CreateTorus(CommandList& commandList, float diameter, float thickness, size_t tessellation, bool rhcoords)
{
	VertexCollection vertices;
	IndexCollection indices;
...
std::unique_ptr<StaticMesh> StaticMesh::CreatePlane(CommandList& commandList, float width, float height, bool rhcoords)
{
	VertexCollection vertices =
	{
...
void StaticMesh::Initialize(CommandList& commandList, VertexCollection& vertices, IndexCollection& indices, bool rhcoords)
{
